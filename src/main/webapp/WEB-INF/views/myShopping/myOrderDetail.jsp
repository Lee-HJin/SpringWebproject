...
<div class="contents">
	...
	<div class="order_detail_info_box">
		<span><b>주문번호:</b> ${orderInfo.orderId }</span>
		<span><b>주문일자:</b> ${orderInfo.handlingDate }</span>
	</div>
	...
	<div class="order_detail_book">
		...
		<table>
			...
			<c:forEach var="dto" items="${lists }">
				<tr>
					<td style="text-align: left; padding-left: 15px;"><a href="<%=cp%>/book_info.action?isbn=${dto.isbn}">${dto.bookTitle }</a></td>
					<td>${dto.quantity }</td>
					<td><fmt:formatNumber value="${dto.bookPrice }" pattern="#,###"/>원</td>
					<td><fmt:formatNumber value="${dto.point }" pattern="#,###"/>원</td>
					<td>${orderInfo.status }</td>
					<td>
						<c:if test="${orderInfo.status!='구매완료' && orderInfo.status!='반품완료' && orderInfo.status!='교환완료' && orderInfo.status!='주문취소'}">
				...
				<tr>
					<td colspan="7" style="text-align: right; padding-right: 10px;"><span style="margin-right: 20px;">합계 : <span style="font-weight: bold; color: #ea5759;"><fmt:formatNumber value="${orderInfo.orderPrice }" pattern="#,###"/>원</span></span><span>적립금 : <span style="font-weight: bold; color: #ea5759;"><fmt:formatNumber value="${orderInfo.point }" pattern="#,###"/>원</span></span></td>
				</tr>
		</table>
		...
		<c:if test="${orderInfo.shipmentsStatusCode == '입금대기중' }">
			<c:if test="${orderInfo.status !='구매완료' }">
				<input type="button" class="order_detail_book_btn2" value="주문취소" onclick="cancelOrder('${orderInfo.orderId}');">
			</c:if>
		</c:if>
		<c:if test="${orderInfo.shipmentsStatusCode == '상품준비중' }">
			<c:if test="${orderInfo.status !='구매완료' && orderInfo.status !='주문취소'}">
				<input type="button" class="order_detail_book_btn2" value="주문취소" onclick="cancelOrder('${orderInfo.orderId}');">
			</c:if>
		</c:if>
		...
		<c:if test="${orderInfo.shipmentsStatusCode == '출고작업중' }">
			<input type="button" class="order_detail_book_btn2" value="반품신청" onclick="returnOrder('${orderInfo.orderId}');">
		</c:if>
		<c:if test="${orderInfo.shipmentsStatusCode == '출고완료' }">
			<input type="button" class="order_detail_book_btn2" value="반품신청" onclick="returnOrder('${orderInfo.orderId}');">
			<input type="button" class="order_detail_book_btn3" value="구매완료" onclick="confirmOrder('${orderInfo.orderId}');">
		</c:if>
		<c:if test="${orderInfo.shipmentsStatusCode == '배송중' }">
			<c:if test="${orderInfo.status !='구매완료' }">
				<input type="button" class="order_detail_book_btn2" value="반품신청" onclick="returnOrder('${orderInfo.orderId}');">
				<input type="button" class="order_detail_book_btn2" value="교환신청" onclick="exchangeOrder('${orderInfo.orderId}');">
				<input type="button" class="order_detail_book_btn3" value="구매완료" onclick="confirmOrder('${orderInfo.orderId}');">
			</c:if>	
		</c:if>
		</div>
	...
	<div class="order_detail_user">
		...
		<table>
			...
			<tr>
				<th>주소</th>
				<td colspan="4">(${orderInfo.zipCode }) ${orderInfo.address1 } ${orderInfo.address2 }</td>
			</tr>
	...
	<div style="margin-top: 20px; margin-left: 10px;">
		<img alt="배송단계" src="<%=cp%>/resources/img/myShopping/my_order_step.gif">
	</div>
